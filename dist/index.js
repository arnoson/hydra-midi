(function () {
    ...
    const chainable = (fn, methods) => {
        ...
        Object.entries(methods).forEach(
            ([name, factory]) => (fn[name] = factory(fn))
        );
        ...
        value(time) {
            ...
            if (elapsedTime < a) {
                ...
                const factor = Math.min(
                    1,
                    (elapsedTime - this.gateDuration) / r
                );
    ...
    const range =
        (fn) =>
        (min = 0, max = 1) =>
            chainable((...args) => map(fn(...args), 0, 1, min, max), {
                scale,
                value,
            });
    ...
    const adsr = (noteId) => () => (a, d, s, r) => {
        [a, d, s, r] = [a, d, s, r].map(
            (arg, i) =>
                arg ?? state.defaults.adsr[i] ?? state.initialDefaults.adsr[i]
        );
    ...
    class SimpleEventEmitter {
        ...
        off(type, handler) {
            this.listeners[type]?.splice(
                this.listeners[type].indexOf(handler),
                1
            );
        }
        ...
        handleMessage(message) {
            if (this.enabled) {
                const { type, data, channel } =
                    MidiAccess.parseMessage(message);
    ...
    const showInputs = (list) => {
        ...
        const template = (input, index) =>
            ...
            `#${index} ` +
            `<span class="hydra-midi-input-name">${getInputName(
                input
            )}</span>` +
            `</div>`;
    ...
    midiAccess.on(
        MidiAccess.TypeAfterTouchChannel,
        ({ input, data, channel }) => {
